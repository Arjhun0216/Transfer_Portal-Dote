...
$sql="UPDATE transfer_details set reg_no=?, name_of_student=?, mobile=?, dob=?, admission_type=?, admission_year=?, admission_month=?, request_for=?, fcollege_c_code=?, fcollege=?, fcollege_autonomous=?, b_name=?, sem_from=?, tcollege_c_code=?, tcollege=?, tcollege_autonomous=?, sem_to=?, last_appeared_month=?, last_appeared_year=?, last_appeared_semester=?, last_appeared_reg_no=?, reason=?, total_periods=?, attended_periods=?, attendence_percentage=?, community=?, quota=?, board=?, mark1=?, mark2=?, mark3=?, cut_off=?,tcr_last_appeared_semester=?,freezed=?,tcollege_sanctioned_intake=?,tcollege_total_students=?,tcollege_total_after=?,status=?,status_reason=?,sanctioned_intake=?,admitted=?,vacancy=? where reg_no=?";
if($stmt=$conn->prepare($sql))
{
    $stmt->bind_param("ssssssssssisissiississiiisssiiiiiiiiissiiis",$_POST['reg_no'],$_POST['name_of_student'], $_POST['mobile'], $_POST['dob'], $_POST['admission_type'], $_POST['admission_year'],$_POST['admission_month'], $_POST['request_for'], $_POST['fcollege_c_code'], $_POST['fcollege'], $_POST['fcollege_autonomous'],$_POST['b_name'], $_POST['sem_from'], $_POST['tcollege_c_code'], $_POST['tcollege'], $_POST['tcollege_autonomous'],$_POST['sem_to'], $_POST['last_appeared_month'], $_POST['last_appeared_year'], $_POST['last_appeared_semester'],$_POST['last_appeared_reg_no'],$_POST['reason'],$_POST['total_periods'], $_POST['attended_periods'], $_POST['attendence_percentage'], $_POST['community'], $_POST['quota'], $_POST['board'], $_POST['mark1'],$_POST['mark2'], $_POST['mark3'], $_POST['cut_off'],$_POST['tcr_last_appeared_semester'],$_POST['freezed'],$_POST['tcollege_sanctioned_intake'],$_POST['tcollege_total_students'],	$_POST['tcollege_total_after'],$_POST['status'],$_POST['status_reason'],$_POST['sanctioned_intake'],$_POST['admitted'],$_POST['vacancy'],$_POST['old_reg_no']);
    if($stmt->execute())
    {
