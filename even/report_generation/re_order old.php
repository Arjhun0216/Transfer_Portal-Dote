...
$sql="select reg_no,name_of_student,address,address2,district,pincode,sem_to,b_name,request_for,fcollege_c_code,tcollege_c_code,attendence_percentage from readmission_details where fcollege_c_code='".$_SESSION['c_code']."' and reg_no='".$_GET['u_id']."' and approved=4";
...
$pdf->SetFont('Arial','B',16);
        ...
        $pdf->Cell(60,8,'Croceedings No:',0,0,'R');
        ...
        $pdf->MultiCell(175,6,'Technical Education - Engineering Colleges - Transfer of students between Institution B.E / B.Tech / B.Arch Part time (B.E / B.Tech / PG / MBA / MCA / MCA(L.E) / M.E / M.tech / M.Sc and B.Sc) Degree Courses - 2024 - 2024 - Even semester (III, V, VII, IX) - Orders Issud - reg.',0,'L',FALSE);
        $pdf->Cell(0,4,'',0,1);
        $pdf->Cell(10,6,'Ref : ',0,0,'L');
        $pdf->MultiCell(175,6,'Transfer Applications forwarded by Principals of Engineering Colleges seeking Transfer of students In even semester',0,'L',FALSE);
        $pdf->Cell(0,8,'****************',0,1,'C');
        
        $pdf->SetFont('Arial','',11);
        $pdf->MultiCell(180,6,'  With reference to the above, transfer is ordered during the even semester of the year 2024-2024, the details of which are given below.',0,'L',FALSE);
        $pdf->Cell(0,4,'',0,1,'C');
$pdf->SetFont('Arial','',12);
...
$x = $pdf->GetX();
$y = $pdf->GetY();
$pdf->Line(10, 165, 10, 120);
$pdf->Cell(60,6,'TRANSFER FROM','LTR',0,'L');
$pdf->SetXY($x + 60, $y);
$pdf->SetFont("Times",'',9);
...
$x = $pdf->GetX();
$y = $pdf->GetY();
$pdf->Cell(60,6,'TRANSFER TO','LRT',0,'L');
$pdf->SetXY($x + 60, $y);
$pdf->SetFont("Times",'',9);
$pdf->MultiCell(0,6,$result['tcollege_c_code']."  ".getCollegeName($result['tcollege_c_code']),1,'L');
$pdf->SetFont('Arial','B',9);
$x = $pdf->GetX();
$y = $pdf->GetY();
