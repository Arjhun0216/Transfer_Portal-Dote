...
<head>
  ...
  <script>
    $(document).ready(function() {
            ...
            if (data == "no_record") {
              ...
              try {
                ...
                $.each(result, function(key, value) {
                  ...
                  var link1 = 'proforma/' + c_code + '_' + $("#reg_no").val() + '.pdf';
                  ...
                  async function checkFileExists1(url) {
                    try {
                      const response = await fetch(url, {
                        method: 'HEAD'
                      });
                      if (response.ok) {
                        isUploadSuccessful = 1;
                        // alert(link);
                        var linkHtml1 = ' <div><label>Click here to view the PDF File</label></div><a href="' + link1 + '" target="_blank" style="color: #007bff;text-decoration: none; font-weight: bold; border: 1px solid #007bff; padding: 5px 10px;margin-left:20px; border-radius: 5px; background-color: #f2f2f2;">' + c_code + "_" + $("#reg_no").val() + '.pdf</a>';
                        $("#displaypdf1").html(linkHtml1);
                        // console.log('File exists');
                      }
                      // else {
                      //     // console.log('File does not exist');
                  ...
                  const fileUrl1 = link1;
                  checkFileExists1(fileUrl1);
              ...
              $("#uploadBtn1").prop("disabled", $("#freeze").is(':checked'));
      ...
      // $("#discontinued_year").change(function(){

      //   $("#readmission_sougth_sem").val(1);
      //   $("#readmission_sougth_sem").val(1);
      // });

      // $("#college_autonomous").change(function(){
      //   if ($("#college_autonomous").val() == "1")
      //               {
      //                 $("#autonomous_years").show();
      //               $("#autonomous_year").val(value["autonomous_year"]);
      //               }
      //               else{
      //                 $("#autonomous_years").hide();
      //               $("#autonomous_year").val(null);
      //               }
      // });
      ...
      $("#uploadBtn1").click(function() {
        var formData = new FormData();
        var files = $('#pdf_upload1')[0].files[0];
        if (files) {
          showLoading();
          formData.append('file', files);
          formData.append('reg_no', $("#reg_no").val());
          $.ajax({
            ...
            success: function(response) {
              ...
              if (response != 0) {
                ...
                $(".modal-content").css("background-color", "#33ff33");
                $("#message").text("File uploaded successfully");
                var linkHtml = ' <div><label>Click here to view the PDF File</label></div><a href="proforma/' + value1 + '.pdf" target="_blank" style="color: #007bff; text-decoration: none; font-weight: bold; border: 1px solid #007bff; padding: 5px 10px; border-radius: 5px; background-color: #f2f2f2;">' + value1 + '.pdf</a>';
                $("#displaypdf1").html(linkHtml);
      ...
      function clearme(isTrue = false) {
        if (isTrue)
          ...
          $("#displaypdf1").html(null);
        $("#pdf_upload").val(null);
        $("#pdf_upload1").val(null);
      }
...
<body>
  ...
  <div class="container">
    <div class="row" style="margin-top: 3%;">
      <div class="box box-warning">
        <div class="box-header with-border">
          <h3 class="box-title">
            Application Form For ODD Sem Re-Admission to UG/PG
          </h3>
        </div>
      </div>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="table-box">
          <div class="table-row table-head" style="padding-bottom: 0px; line-height: 5px;">
            <div class="table-cell first-cell" style="padding-top: 5px; padding-bottom: 0px;">
              <p>Description</p>
            </div>
          ...
          <div class="table-row">
            ...
            <div class="table-cell">
              <select class="form-control" id="discontinued_sem" style="margin: 5px auto; width: 50%;">
                <option></option>
          ...
          <div class="table-row">
            ...
            <div class="table-cell">
              <!-- <input type="text" name="" id="readmission_sougth_sem" class="form-control" style="margin: 5px auto; width: 50%;" readonly> -->
            ...
            <div class="table-cell last-cell">
              <input type="text" class="form-control" id="readmission_sougth_year" value="2025-26" style="margin: 5px auto; width: 50%;" readonly />
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="row" style="margin-top: 2%;">
      <div class="col-md-6">
        <div class="form-group">
          <div class="col-sm-8">
            <label for="reason_discontinuance" class="control-label">Reason for Discontinuance of study</label>
          </div>
          <div class="col-sm-10">
            <textarea autocomplete="off" class="form-control" id="reason" name="reason_discontinuance" rows="2" columns="100"></textarea>
          </div>
        </div>

        <div class="form-group">
          <div class="col-sm-10">
            <label for="lack_attendance" class="control-label">Mention the odd Semester in which the student was put into lack
              of attendance(III, V, VII, IX)
            </label>
          </div>
          <div class="col-sm-6">
            <!-- <input type="text" name="" id="lack_attendence_sem" class="form-control" readonly> -->
        ...
        <div>
          <div class="form-group">
            <label><b>Upload proforma - 1 to 5 (if autonomous)</b></label>
            <form method="post" action="" enctype="multipart/form-data" id="uploadForm1">
              <div>
                <input required="" id="pdf_upload1" name="pdfFile" type="file" accept=".pdf"><br>
                <button id="uploadBtn1" type="button" style="background-color: dodgerblue; color: white; border-radius: 6px; padding: 5px; margin-top: 10px;margin-left: 10px;">Upload</button>
              </div>
            </form>
          </div>
          <div class="form-group">
      ...
      <div class="col-md-6">
        <div class="form-group">
          <div class="col-sm-10">
            <label for="sem_attendance" class="control-label">Total No. of Periods taken into account for calculation of
              attendance for the above Semester</label>
          </div>
          <div class="col-sm-6">
            <input autocomplete="off" class="form-control" id="total_periods" name="sem_attendance" type="text" value="" />
          </div>
        </div>
        <div class="form-group">
          <div class="col-sm-6">
            <label for="att_periods" class="control-label">No. of Periods attended by the student</label>
          </div>
          <div class="col-sm-6">
            <input autocomplete="off" class="form-control" id="total_attended" name="att_periods" type="text" value="" />
          </div>
        </div>

        <div class="form-group">
          <div class="col-sm-6">
            <label for="att_per" class="control-label">Percentage of the Attendance</label>
          </div>
          <div class="col-sm-6">
            <input autocomplete="off" class="form-control" id="percentage" name="att_per" type="text" value="" />
          </div>
          <div class="col-sm-6">
      ...
      <div class="col-md-12">
        <div class="box box-primary">
          <div class="box-header with-border text-center">
            <button id="submit" class="btn btn-primary waves-effect waves-light" type="submit" value="Submit" name="actionType">
            ...
            <button class="btn btn-success" id="home"><a style="text-decoration:none;color:white;" href="../user/#from">Home</a>
            </button>
            <a href="../user/#readmission" class="btn btn-info">Back</a>
          </div>
        </div>
      </div>
    </div>
